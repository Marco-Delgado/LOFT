...
class League(BaseEnv):
    ...
    def __init__(self, config):
        ...
        # Types
        self.object_type = structs.Type("object")
        self.pos_type = structs.ContinuousType("pos")
        self.pos_type.set_sampler(lambda rng: rng.uniform(-10, 10, size=4))

        # Predicates
        self.IsObject = structs.Predicate(
            "IsObject", 1, is_action_pred=False,
            holds=self._IsObject_holds, var_types=[self.object_type])
        self.On = structs.Predicate(
            "On", 2, is_action_pred=False,
            holds=self._On_holds,
            var_types=[self.object_type, self.object_type])
        self.OnTable = structs.Predicate(
            "OnTable", 1, is_action_pred=False,
            holds=self._OnTable_holds, var_types=[self.object_type])
        self.Clear = structs.Predicate(
            "Clear", 1, is_action_pred=False,
            holds=self._Clear_holds, var_types=[self.object_type])
        self.Holding = structs.Predicate(
            "Holding", 1, is_action_pred=False,
            holds=self._Holding_holds, var_types=[self.object_type])
    ...
    def get_state_predicates(self):
        return {self.IsObject, self.On, self.OnTable, self.Clear, self.Holding,
                self.HandEmpty}
    ...
    def _sample_initial_piles(self, num_objs):
        ...
        for block_num in range(num_objs):
            block = self.object_type(f"object{block_num}")
    ...
    def _sample_state_from_piles(self, piles):
        ...
        # Create ROBOT state
        robot_state = {}
        robot_state["eef_pos"] = [0.9, 0.3, 0.3]
        ...
        for block, pile_idx in block_to_pile_idx.items():
            ...
            object_state = {"pos" : [x, y, z], "grasped" : False, "on_table" : True}
            state[block] = object_state
        return state
    ...
    @staticmethod
    def _sample_xy(rng):
        x = rng.uniform(League.x_lb, League.x_ub)
        y = rng.uniform(League.y_lb, League.y_ub)
        return x, y
    ...
    def _get_next_state_put_on_table(self, state, action):
        ...
        next_state[holding_obj]["grasped"] = False
        return next_state
    ...
    @staticmethod
    def _IsObject_holds(state, obj):
        return obj in state and obj.var_type == "object"
    ...
    def _On_holds(state, obj1, obj2):
        ...
        if obj1 is None or obj2 is None:
            return False
        result = obj2[0] - 1 < obj1[0] < obj2[0] + 1 \
                 and obj2[1] - 1 < obj1[1] < obj2[1] + 1
        return result
    ...
    def _Holding_holds(state, obj):
        ...
        assert holding == (state[WORLD]['block_to_pile_idx'][obj] \
                           is None)
        assert holding == state[obj]["grasped"]
        return holding
    ...
    def _put_on_table_sampler(cls, rng, state, *args):
        del state  # unused
        del args  # unused
        (x, y) = League._sample_xy(rng)
        pos = [x, y, League.table_height + \
                0.5 * League.block_size]
        return (pos,)
    ...
    def _state_has_forbidden_collisions(self, state, grasped_obj_constraint=None,
                                        interacting_objs=None):
        # Set state
        self._reset_pybullet_from_state(
            state, grasped_obj_constraint=grasped_obj_constraint)
        ## Get aabbs
        # Get object aabbs
        obj_to_aabb = {}
        ...
        # Check collisions between WORLD and non-grasped object
    ...
    def _reset_pybullet_from_state(self, state, grasped_obj_constraint=None):
        ...
        if grasped_obj_constraint is None:
            finger_val = 0.035
        else:
            finger_val = 0.025
        ...
        for obj, py_obj in self._obj_to_pybullet_obj.items():
            if grasped_obj_constraint is not None and \
                obj == grasped_obj_constraint[0]:
                pose, orn = self._apply_pick_constraint(grasped_obj_constraint)
            else:
                pose = state[obj]["pose"]
                orn = [0, 0, 0, 1]
            p.resetBasePositionAndOrientation(
    ...
    def _rebuild_pybullet_objects(self, state):
        ...
        objects = sorted([o for o in state if o != WORLD])
